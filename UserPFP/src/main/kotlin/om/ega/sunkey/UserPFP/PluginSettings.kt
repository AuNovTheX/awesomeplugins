...
class PluginSettings(private val settings: SettingsAPI) : SettingsPage() {
	...
	val p = DimenUtils.defaultPadding
        val textInput = TextInput(view.context, "Refresh UserPFP db time (min) / Actualizar la base de datos de UserPFP (min)") //texthint finally 
        ...
        refreshCache.setOnClickListener { Button: View? ->
            Utils.threadPool.execute {
                ...
                context?.let { UserPFP.USRPFP.getCacheFile(it) }?.let { UserPFP.USRPFP.downloadDB(it) }
        ...
        addView(
            createCheckedSetting(
                view.context,
                "Prioritize Nitro PFP over USRPFP",
                "nitroBanner",
                true
            )
        )
